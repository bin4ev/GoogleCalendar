...
export class DayCalendarComponent {
  ...
  createEvent(e: any) {
    ...
    let formatEnd = Number(h) + 1 > 12 ? 'PM' : 'AM'
    this.createEventStart = {
      name: ('(No title)'),
      start: `${h}:${m} ${format}`,
      end: `${Number(h) + 1}:${m} ${formatEnd}`,
      duration: 60,
      createdBy: user.name,
      color: user.color,
      top: topPos,
  ...
  setPositionScrow() {
    this.createdEvent.nativeElement.scrollIntoView(true)
  }
